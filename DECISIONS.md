...
- Date: 2026-08-01
- Status: Proposed — Security design review PASS at `5e904c6`; fresh Human architecture approval required before Issue #133 implementation resumes
...
- Decision: On initial closure, store the exact accepted active revision once and append immutable digest-linked peers. `docs/status/manifest.yaml` binds each identity's active digest, lineage-head digest, next sequence, generation, and whole-set digest under its own digest. Feature worktrees may validate proposals against expected default commit/manifest/set/head but cannot reserve paths or establish acceptance. A two-party-approved, least-privilege controlled writer rebuilds on the current protected default tip, performs TOCTOU-safe path handling, creates one commit containing records/manifest/projection, and publishes by ordinary non-force ref update from that exact parent. The Git server ref CAS is the serialization point; stale/racing proposals reload, receive fresh approval, and allocate new paths without auto-merge. Preserve the SDD's strict parser/JCS/resource/data/fixture controls and `phase:not_applicable`. Hashes prove connectivity but require an external commit/digest anchor to detect wholesale replacement.
...
- Consequences: Archive storage is Θ(n), but archived-only verification requires the complete bounded peer bundle and reports when unanchored. Local validation and local commits are never authoritative acceptance. The current loader/schema are replaced, not retained on the authoritative path. Security Review PASS is evidence for Human architecture review only. Human must still approve ADR-0018, correction approvers, branch protection, controlled-writer credentials, external anchor, limits/runtime/retention, migration exceptions, and switch/rollback before implementation resumes. Routine deletion remains forbidden; incident purge invalidates digests/anchors. ADR-0017's one-authority-per-phase rule remains unchanged.
