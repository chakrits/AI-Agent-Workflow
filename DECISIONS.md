...
- Decision: On initial closure, move the exact accepted active revision unchanged into the existing identity archive directory and append a separate immutable closure peer linked by `supersedesDigest`. Corrections append one immutable peer using expected lineage-head and set digests; stale/racing corrections fail and retry from the latest head without auto-merge. Validate archive bundles iteratively and explicitly; default active loading performs zero archive I/O. Apply the frozen raw/aggregate/memory/output limits, strict YAML-to-JSON boundary, normative restricted RFC 8785 algorithm/vectors, safe evidence/data-retention policy, exact error precedence, and versioned executable fixture manifest in the SDD. `phase:not_applicable` is the sole non-lifecycle sentinel. Hashes prove internal connectivity but cannot detect wholesale replacement of an unanchored bundle; an independently retained expected digest is required for that assurance.
...
- Consequences: Archive storage is Θ(n), but archived-only verification requires the complete bounded peer bundle and reports when it lacks an external anchor. The pre-activation layout and existing implementation/tests are superseded evidence, not authority. Developer work requires prior Security Reviewer and Human approval, then flat-peer schema/path/transition behavior, strict parser/JCS/data-policy controls, fixture manifest/runners, migration/rollback tests, and all Slice-B gates. Routine deletion is forbidden; sensitive-data purge is a separately approved security incident because it invalidates digests/anchors. ADR-0017's one-authority-per-phase rule remains unchanged.
