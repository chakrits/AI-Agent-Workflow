...
### ADR-0018: Store Canonical Predecessors as Flat Immutable v1 Archive Peers

- Date: 2026-08-01
- Status: Accepted, amended — Human Maintainer approved bounded foundation implementation on 2026-08-01 and narrowed the increment runtime support/merge gate on 2026-08-02; authority switch, rollback activation, and Go remain unapproved later checkpoints
- Context: The original `work-item-status/v1` draft deleted the active predecessor, allowed closure fields to differ, and retained only `supersedesDigest`, so archived-only validation lacked the genuine digest preimage. Commit `46ffe7f` proposed recursively embedding the full predecessor in each successor. Independent review found that n corrections then retain Θ(n²) aggregate bytes, repeatedly copy sensitive mistakes, force recursive parsing, and complicate deletion. The design also lacked raw/parser limits, explicit correction concurrency, data policy, cross-runtime canonicalization, and an executable fixture contract.
- Decision: On initial closure, store the exact accepted active revision once and append immutable digest-linked peers. `docs/status/manifest.yaml` binds each identity's active digest, lineage-head digest, next sequence, generation, and whole-set digest under its own digest. Feature worktrees may validate proposals but cannot reserve paths or establish acceptance. Every correction requires explicit Human Maintainer approval bound to its semantic delta and current digests. GitHub Actions is the controlled writer, using least-privilege short-lived credentials with rotation/revocation, on a protected default branch with no bypass: Actions prepares and validates the candidate against the current merge-queue/default-branch base, while protected merge-queue/default-ref acceptance is the serialization point. Stale/racing proposals reload, receive fresh approval, and allocate new paths without auto-merge. External anchor custody is separate from the writer. Preserve strict parser/JCS/resource/data/fixture controls, repository-lifetime retention with incident-only Security+Human purge, unrecoverable-predecessor fail-closed handling, and `phase:not_applicable`. For Issue #133/PR #135 increment verification, the supported merge gate is local macOS Node 22 plus hosted Ubuntu Node 22 and Python 3.12. Windows is explicitly N/A/deferred and unsupported for this increment; whole-repository Windows portability is separately owned by the Human Maintainer/repository backlog.
- Alternatives Considered: Recursively embed predecessor preimages (rejected after rework because aggregate storage is Θ(n²), privacy exposure is replicated, and parsing/deletion are harder); persist only an API transition log (rejected because a digest assertion cannot prove absent predecessor bytes, while trusted signatures/transparency add unapproved infrastructure); use a second content-addressed object namespace (rejected because the existing identity archive can store each peer once without new GC/orphan authority); reconstruct predecessor fields (rejected as unsound); rely on Git history alone (rejected because archived-only verification must work from an exported bundle).
- Consequences: Archive storage is Θ(n), but archived-only verification requires the complete bounded peer bundle and reports when unanchored. Local validation and local commits are never authoritative acceptance. The current loader/schema must be replaced, not retained on the authoritative path. Developer may implement only the bounded foundation: parser/JCS/schema/loader replacement, manifest and flat-peer transition logic, versioned fixtures, and disposable CAS/TOCTOU harnesses. Consumer migration, production projection/writer activation, hosted credentials, authority switch, rollback activation, compatibility removal, release, and Go remain unauthorized until shadow evidence, independent review/QA/Security, and a separate Human checkpoint. Windows evidence is retained historically: Python 3.12 passed, focused status tests passed after portability fix `52c7b8c`, and six unrelated full-suite tests failed; none of these facts makes Windows supported or blocks this increment's merge. Routine deletion remains forbidden; incident purge invalidates digests/anchors. ADR-0017's one-authority-per-phase rule remains unchanged.
