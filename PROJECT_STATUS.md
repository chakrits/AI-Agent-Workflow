...
- Issue #212 (IMP-006) design decision recorded as [ADR-0021](DECISIONS.md) (2026-09-07): Option B (body-below-frontmatter parity, `name:` pinned equal) approved for AC-01; `role-definitions.md`'s Terminal Dispatch and Boss Visibility section approved for relocation into `task-execution-mode.md` for AC-05 (≈615 tokens headroom expected, verify via `npm run validate:context-budget` after the edit — not yet performed). QA's Cross-Platform Acceptance Criteria Gate relocation explicitly declined for now. AC-02 through AC-04, AC-06 through AC-08 (the parity script, the actual relocation edit + `repin:source-matrix`, adapter wiring, the Release Agent skill) are not yet implemented — this ADR unblocks Developer dispatch, it does not complete the Issue.
