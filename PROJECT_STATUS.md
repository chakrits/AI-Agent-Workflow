...
## In Progress
- Issue #166 implementation is ready for independent QA re-review: SUBAGENT-02 added templates and compact canonical pointers ([Issue comment 5263728633](https://github.com/chakrits/AI-Agent-Workflow/issues/166#issuecomment-5263728633)), and the Documentation Agent then cleared five independent-QA findings on draft PR #167 — corrected the two red required-check markers in the PR body, added the canonical runtime/receipt state glossary for AC-07, relocated the durable normative content to `docs/workflow/task-execution-mode.md` so it survives `npm run reset:template`, wired the mode into the `dynamic-workflow` skill, and added the AC-06 counter to `docs/templates/TASK_REVIEW.md`. `phase:development` and `status:spec-ready` are unchanged.
...
- Detailed lightweight specification: `docs/records/sdd/2026-08-12-issue-166-task-execution-mode-spec.md` — approved (historical record only)
- Canonical mode definition: `docs/workflow/task-execution-mode.md` — permanent normative source, outside the reset-cleared directories
