...
- Resolved by Human Maintainer decision (2026-09-07, [ADR-0020](DECISIONS.md)): accepted SA Agent's recommendation to keep `scripts/validate-ci-parity.mjs`'s `githubJobCommands` zero-command guard as-is — it is a single shape-independent postcondition, not a per-shape enumeration, and round 3's own adversarial probe found no gap. No code change. The Issue #210 round-3 design Question is closed.
- Deferred by Human Maintainer decision (2026-09-07, [ADR-0020](DECISIONS.md)): SA Agent's separately-surfaced `normaliseCommand`/`IGNORED_COMMAND_RE` finding (a `run: |` multi-line step's non-global `.match()` only extracts the first command, and a block whose first line matches `IGNORED_COMMAND_RE` is discarded whole) is confirmed latent, not currently live — every `run:` step in `.github/workflows/validate-contracts.yml` is single-line. Deferred rather than fixed now; revisit if a multi-line `run:` block is ever introduced.
