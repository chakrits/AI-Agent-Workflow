...
- Design Question from Issue #210's round-3 QA: `scripts/validate-ci-parity.mjs`'s `githubJobCommands` enumerates shapes that yield zero commands (missing job, empty steps, composite-action-only) rather than positively asserting job validity. SA Agent evaluated this on 2026-09-07 and recommended keeping the current pattern as-is — rereading the function, it is a single shape-independent postcondition (`commands.size === 0` → throw), not a per-shape enumeration, and round 3's own adversarial probe (four new shapes) found no gap. SA separately surfaced a different, latent (not currently live) risk one level down in `normaliseCommand`/`IGNORED_COMMAND_RE`: a `run: |` multi-line block's non-global `.match()` only extracts the first command, and a block whose first line matches `IGNORED_COMMAND_RE` (e.g. `npm ci`) is discarded whole, silently dropping any real commands after it. Not exercised today — every `run:` step in `.github/workflows/validate-contracts.yml` is single-line. Owner: Human Maintainer, to decide (a) accept the `githubJobCommands` recommendation and close this item, and (b) whether the `normaliseCommand` follow-up is worth fixing now or left until a multi-line `run:` block is actually introduced.
