...
The `correlation` object may contain only these typed fields: `handoff_event_id`, `terminal_result_id`, `pair_id`, `measurement_id`, and `parent_event_id`. Each value is a non-empty string. `handoff_event_id` uses the existing receipt identifier shape; `terminal_result_id` is required only when a terminal result exists; `pair_id` is required only for the paired shadow-result events `shadow_compared`, `shadow_fallback`, and `rollback_completed`; `measurement_id` is required for route/context/baseline/outcome observations, including `context_loaded` when its authority is `shadow`; `parent_event_id` is required for a child event in an event chain. Extra correlation keys are invalid.
...
| `context_loaded` | `workflow_record` | `legacy` or `shadow` | `measurement_id` (always; `pair_id` is forbidden) | `context_mode`, `source_manifest_digest`, `token_measurement_status`; `approximate_tokens` when available | `success`, `failure`, or `inconclusive`; reason for non-success; top-level `digest_ref` required |
...
Shadow observations use the same envelope; they are not a second record shape. For the paired shadow-result events `shadow_compared`, `shadow_fallback`, and `rollback_completed`, `authority=shadow`, `correlation.pair_id` is required, and all event-specific values are under `attributes`. `context_loaded` is intentionally excluded from this paired-result rule: even when its authority is `shadow`, it records one context-load observation and uses `correlation.measurement_id`, as its event row explicitly requires. The value `legacy_result_digest` identifies the legacy observation being compared; it does not change the envelope authority to `legacy`. The top-level `digest_ref` is required for `shadow_compared` only in this v1 mapping and must not be copied into `attributes`.
