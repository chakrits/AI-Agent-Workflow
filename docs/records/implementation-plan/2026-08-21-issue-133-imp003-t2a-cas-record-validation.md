...
The `Reviewed Candidate SHA` is the exact immutable commit under SA/QA review and the authoritative review target. The separate `Handoff Record Commit SHA` identifies the commit containing the handoff record. If embedding the record's own final SHA would create a self-reference, the handoff must state that the record commit is resolved externally as the branch final SHA or identified by its parent commit. Historical verification points are evidence only, never current targets. `PENDING` and `NEEDS_REVISION` are invalid placeholder values for both fields.
...
Five vectors are frozen from `test/status-audit.test.mjs:211-271` at prior candidate verification point `5b657d3bb1902c0e1db7b24e4b9202ab888444d5` and `scripts/lib/status-audit.mjs:259-340`, and are materialized with stable IDs, exact canonical strings, and outputs in `test/fixtures/status-cas/v1/digest-vectors.json`: `T2A-DIGEST-001/MANIFEST` → `11ee19ff51b96ea45af5080ba14d8b4513772d95219ae21086781f4c58b4c88c`; `/SET` → `9bea80eaae94dfd06301903d9b5f3d7740221794495160800edac7cbb137f600`; `/HEAD` → `a37343194a1ac035cdfb7fb1c3d94a1abd55b1237777dec27ac702518ebefe8d`; `/PROJECTION` (`a\r\nb\r\n` → `a\nb\n`) → `911169ddaaf146aff539f58c26c489af3b892dff0fe283c1c264c65ae5aa59a2`; `/CONTENT-TREE` → `df3e425dec1ba06274a95db2364dfc7e66c769c9a5e3982ff1204fb1452ea45a`. The current SA review target remains the separate `Reviewed Candidate SHA` above.
