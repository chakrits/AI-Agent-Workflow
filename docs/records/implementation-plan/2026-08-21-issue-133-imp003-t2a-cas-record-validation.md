...
The manifest path/format and omission procedure are frozen, but its exact case IDs, category-to-ID mapping, numeric `caseCount`, and canonical manifest digest are not safely derivable from T1 because no T2 corpus or runtime expected-error set exists. This is a hard context stop, not a guess: return `NEEDS_CONTEXT` and request SA/Human approval of the synthetic record vectors plus the complete numbered subcase list. Only then may the final manifest preimage/digest be computed and `status:spec-ready` considered.

### Revised task dependencies and rollback boundary

`T2A-01` now ends with the SA/Human decision above. `T2A-02` and `T2A-03` cannot begin implementation fixture authoring until the complete manifest list and record-vector decision are approved. All later tasks remain sequential. Rollback remains documentation-only before approval; after approval, revert only T2-A runtime/schema/fixture files and rerun unchanged T1 tests. No writer, publication, ref, authority, replay store, credential, lifecycle, or T2-B boundary is introduced.
