...
- Owner: SA Agent
- Status: Architecture accepted for bounded foundation implementation; authority switch, rollback activation, and Go remain gated
...
- No hosted authority switch, rollback activation, release, or Go/No-Go decision.
...
The GitHub Actions controlled writer operates only in a clean, newly created checkout of merge-queue/default-branch commit `C`. It receives expected `C/M/S/H`, a reviewed transaction, and approval evidence. It re-reads and validates manifest and paths immediately before mutation; opens/creates within a canonical allow-listed root using no-follow/exclusive-create primitives where supported; verifies regular-file identity after open; writes new files to same-filesystem temporary paths; fsyncs files/directories where supported; constructs one Git index/tree without exposing a partially accepted tree; and revalidates `C/M/S/H`, path absence, inode identity, and approval freshness. Actions publishes the candidate only to a dedicated integration branch/PR and has no protected-default bypass. Required checks rebuild and validate against the merge queue's current base. Protected merge-queue/default-ref acceptance is the serialization point: a candidate based on stale `C/M/S/H` is rebuilt or rejected before merge. The accepted default-branch commit contains archive/active mutations, manifest, and projection together. Filesystem rename, a local Git commit, or an integration-branch push alone is not authoritative acceptance.
...
Every correction requires explicit Human Maintainer approval; the requestor cannot substitute self-approval. Security Reviewer approval is additionally required for data-policy, retention, redaction, purge, or credential-impacting corrections. Approval binds identity, requested semantic delta, `C/M/S/H`, expiry, and request ID; stale retry invalidates it. GitHub Actions is the controlled writer. It uses least-privilege short-lived credentials with documented rotation/revocation, protected-environment access, no credential exposure to feature-worktree code, and auditable request/approval/candidate/accepted-commit IDs. Default-branch protection has no writer bypass: Actions prepares a candidate on the current merge-queue/default-branch base and satisfies required checks; protected merge-queue/default-ref acceptance is the authoritative CAS. External anchor custody is organizationally and credential-separate from Actions. Concrete hosted rules, credential owner, anchor custodian, trigger, and compromise runbook require activation review before live mutation.
...
10. A→B and B→A initial/correction races prove one winner, stale loser, explicit reapproval/retry with new sequence, exact union, and no branch/overwrite/loss; TOCTOU path/inode swaps fail before acceptance. At/beyond sequence-boundary fixtures prove `ARCHIVE_SEQUENCE_EXHAUSTED`, no wrap/reuse/width expansion, and zero side effects.
...
The accepted increment runtime/support matrix is Node 22.x on local macOS and hosted Ubuntu, plus an independent Python 3.12 reference verifier for JCS vectors on hosted Ubuntu. These supported cells must match exact canonical bytes/digests/errors; Node filesystem side-effect cases run on macOS and Ubuntu. Windows is explicitly unsupported/deferred for this increment by Human Maintainer decision on 2026-08-02 and is N/A at the merge gate, not a silent pass: historical PR #135 evidence remains that Python 3.12 on Windows passed and the focused status tests passed after portability fix `52c7b8c`, while six unrelated whole-repository full-suite tests failed. Whole-repository Windows portability is a separate Human Maintainer/repository-backlog follow-up. Manifest schema, fixture files, and runner remain versioned together; changing expected bytes/digests/errors requires architecture review and a manifest-version changelog.
...
- Authorization/integration: explicit Human Maintainer correction approval, short-lived least-privilege controlled-writer credentials with rotation/revocation, protected no-bypass default-ref CAS, separately custodied external anchors, digest-covered manifest heads, and TOCTOU-safe path handling.
...
- Accepted decision (ADR-0018, Human approval 2026-08-01): retain each complete predecessor once as a flat immutable peer revision, use the digest-covered manifest/protected-ref CAS, and reject embedded recursive chains, transition-log-only, reconstructed-predecessor, and Git-history-only models for v1.
...
- Resolved Human decision: bounded foundation implementation is approved with the frozen raw/aggregate/file-count/memory/output/JCS limits; this is not authority-switch or Go approval.
- Resolved Human decision: an unrecoverable predecessor fails closed; only a recorded Human migration exception may disposition it, and reconstruction/silent waiver remain forbidden.
- Resolved Human decision (2026-08-02, Issue #133/PR #135): the increment merge gate is local macOS Node 22 plus hosted Ubuntu Node 22 and Python 3.12. Windows is N/A/deferred and unsupported for this increment; its historical partial passes and six unrelated full-suite failures remain evidence, not acceptance. Whole-repository Windows portability is owned by the Human Maintainer/repository backlog.
- Resolved Human decision: repository-lifetime retention; purge only for a Security incident with Security Reviewer and Human approval.
- Review evidence resolved: final Architecture review `ddcf373` and final CAS Security re-review `16c3ecf` are PASS evidence consumed by the Human Maintainer's bounded-foundation approval. Neither review independently authorizes implementation, hosted activation, authority switch, rollback activation, release, or Go.
- Resolved Human decision: Human Maintainer approves corrections; GitHub Actions is the least-privilege controlled writer on protected default branch with no bypass and rotation/revocation; external anchor custody is separate from the writer. Exact hosted identities/configuration remain activation artifacts, not foundation implementation authority.
- Resolved rework evidence: CP1-133-01 inventory reconciled to 61 unique files / 62 categorized surfaces at `786df83`; final independent Architecture and Security reviews passed before Human acceptance.
...
- Human gate: ADR-0018/CAS is accepted for bounded foundation implementation. Authority switch and rollback activation require completed shadow compatibility evidence, independent Reviewer/QA/Security evidence, and a separate Human checkpoint. This record does not authorize live writer activation, authority switch, rollback execution, release, or Go.
