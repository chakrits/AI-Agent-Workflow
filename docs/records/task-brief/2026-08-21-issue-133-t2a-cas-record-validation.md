...
| Approval binding input | `record`, `identity`, `independent`, `proposal`, `predecessor`, `result`, `consumedRecordDigests` | `record` must validate first; `independent` must be `false`; identity equals `record.predecessor.authenticatedBy`; proposal/predecessor/result equal record proposal/predecessor.digest/successor. Public JSON and serialized fixtures use `consumedRecordDigests` as an array of lowercase 64-hex strings only; the runtime boundary copies it into a fresh `Set` for membership in this call. `null`, a missing value where the closed approval input requires the field, or any non-array container rejects with `INVALID_RECORD`; malformed members reject with `INVALID_RECORD`; duplicate digests are accepted as idempotent input and collapse in the internal `Set`; array order is ignored for membership and the caller array is never mutated. |
...
- The fixed vector IDs above and their exact canonical bytes/digests must be checked into the authoritative checked-in digest-vector corpus. `digest-vectors.json` preserves the existing T1-compatible top-level `derived` object alongside the canonical `vectors` map. `derived` keeps the existing focused-test output contract, while `vectors` remains the T2-A named-vector map; this compatibility alias is fixture-only and requires no production or implementation-test edit.

Replay means only validating a duplicate/reused record or approval input against caller-supplied data such as `consumedRecordDigests`. T2-A must not create or read a replay store, mutate a consumed/used flag, track publication state, or perform an authority/identity capability check. `consumedRecordDigests` is a public JSON array and test-fixture representation, converted once at the runtime boundary to a fresh internal `Set`; it is not a public `Set`, is never serialized as one, and is never written back. A repeated input is rejected as `APPROVAL_REPLAY` only when the supplied array contains its digest; duplicates are semantically idempotent, ordering is irrelevant, and the pure function does not add the digest. JSON object key order is irrelevant only to JCS digest canonicalization; `changedPaths` order remains significant because it is part of the record preimage.
