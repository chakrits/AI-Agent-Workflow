...
- Bring `.agent/skills/` — the documented "Antigravity CLI skills" adapter (`AGENTS.md:135`) — to full parity with `.agents/skills/`: mirror the four new skills and the updated `qa-playwright-testing` (with its new Accessibility Testing section), **and** backfill the four skills already missing there before this work (`ba-requirement-analysis`, `data-config-change`, `sa-architecture-design`, `security-review`) as straight copies of their existing `.agents/skills/` content — no new authoring, since that content already exists and is already approved policy.
...
- Mirroring any skill into `.codex/` — unlike `.agent/skills/`, `.codex/` does not mirror any skill today (it holds a single `orchestrator-supervision.md`, a different adapter shape entirely), so there is no existing parity pattern to extend there. Full `.agent/skills/` parity (in scope, above) does not imply `.codex/` parity — that would be a new adapter shape, not a backfill.
...
`.agent/skills/` mirrors a subset of `.agents/skills/` for the Antigravity CLI host (`AGENTS.md:135`), but had never been kept in step as role-specific skills were added to `.agents/skills/` — it currently only carries the 11 generic/cross-role skills. This spec closes that gap in full, not just the QA-relevant slice:
...
3. Backfill the four skills that were already missing from `.agent/skills/` before this work — copy `.agents/skills/ba-requirement-analysis/SKILL.md`, `.agents/skills/data-config-change/SKILL.md`, `.agents/skills/sa-architecture-design/SKILL.md`, and `.agents/skills/security-review/SKILL.md` into `.agent/skills/` unchanged. This is a mechanical copy of already-approved, already-shipped content — no new policy or prose is authored here; it is fixing a stale mirror, not writing new rules.

After this spec, `.agent/skills/` and `.agents/skills/` carry the identical set of 20 skills (11 pre-existing generic + 5 pre-existing role-specific + 4 new).
...
7. `.agent/skills/` contains all 20 skills that `.agents/skills/` contains, byte-identical content per file: the four new skills, `qa-playwright-testing` (with Accessibility Testing), and the four backfilled skills (`ba-requirement-analysis`, `data-config-change`, `sa-architecture-design`, `security-review`).
...
9. `test/validate-contracts.test.mjs` has regression coverage: the new Skill Routing rows, the `Test Effectiveness` rule, each new skill file's presence and key content under both `.agents/skills/` and `.agent/skills/`, full `.agent/skills/`↔`.agents/skills/` parity (a test that fails if any skill directory name present in one is absent from the other), the `qa-playwright-testing` accessibility section in both locations, the four new `SKILL_CATALOG.md` entries, and both template changes each fail the test if removed.
...
- Mirroring into `.agent/skills/` introduces a second copy of each skill file's content — nine files now, not two (four new QA skills + `qa-playwright-testing` + four backfilled skills). This repo already accepts this duplication cost for eight of the eleven generic skills mirrored across both directories as full copies; this spec brings the role-specific skills to that same standard. There is still no tooling that keeps the two directories in sync automatically — the new regression test (Acceptance Criteria 9) only catches drift after the fact for the files it checks, it does not prevent a future single-directory edit. A skill-sync tool remains out of scope here.
- Discovered during implementation (not caught at design time): three pre-existing generic skills — `dynamic-workflow`, `frontend-ui-engineering`, `functional-test-design` — use a thin pointer-adapter pattern in `.agent/skills/` ("This is a platform adapter. The canonical source is: ...") instead of a full-copy mirror, unlike the other eight generic skills. This predates and is unrelated to this work. Acceptance Criteria 9's parity test is scoped to only the 9 skills this spec touches for exactly this reason — it does not assert byte-identical content for those three, and this spec does not alter them. Reconciling the two coexisting `.agent/skills/` patterns (full-copy vs. thin-pointer) is a separate, out-of-scope decision.
