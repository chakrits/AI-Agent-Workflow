...
For asynchronous dispatches, the parent Orchestrator registers a native child-terminal await/callback after successful invocation and before the parent ends or yields. The receipt records `Handoff Event ID`, `Parent Orchestrator ID`, `Child Task ID`, and `Completion Event Evidence`. When the host resumes the parent with a `Terminal Result ID`, the parent consumes `(Handoff Event ID, Terminal Result ID)` exactly once, emits one Boss event, and records the permitted route or stop in `Consumption Evidence`. The completion primitive has no authority to make a workflow decision. If parent retention/resumption is unavailable, record `host_completion_unavailable`; deadline expiry and explicit cancellation record `timed_out` or `cancelled` as evidence-backed terminal outcomes. A heartbeat is diagnostic-only after a block and cannot route work.
